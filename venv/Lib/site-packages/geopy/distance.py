...
class geodesic(Distance):
    ...
    def set_ellipsoid(self, ellipsoid):
        ...
        if isinstance(ellipsoid, str):
            try:
                ...
                raise Exception("Invalid ellipsoid. See geopy.distance.ELLIPSOIDS")
        else:
    ...
    def measure(self, a, b):
        ...
        if not (
            isinstance(self.geod, Geodesic)
            and self.geod.a == self.ELLIPSOID[0]
            and self.geod.f == self.ELLIPSOID[2]
        ):
            self.geod = Geodesic(self.ELLIPSOID[0], self.ELLIPSOID[2])

        s12 = self.geod.Inverse(lat1, lon1, lat2, lon2, Geodesic.DISTANCE)["s12"]
    ...
    def destination(self, point, bearing, distance=None):
        ...
        r = self.geod.Direct(
            lat1, lon1, azi1, distance, Geodesic.LATITUDE | Geodesic.LONGITUDE
        )
